                ...
                <?php foreach ($gurus as $idx => $g): ?>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <small class="text-muted"><?= esc($g['nip_nik'] ?: '-') ?></small>
                        </td>
                        <td><span class="badge bg-light text-dark border"><?= esc($g['posisi']) ?></span></td>

                        <?php if ($pRecord): ?>
                            <td><span class="fw-bold text-success"><?= $pRecord['skor_pilar_1'] ?></span>%</td>
                            <td><span class="fw-bold text-success"><?= $pRecord['skor_pilar_2'] ?></span>%</td>
                            <td><span class="fw-bold text-success"><?= $pRecord['skor_pilar_3'] ?></span>%</td>
                            <td><span class="fw-bold text-success"><?= $pRecord['skor_pilar_4'] ?></span>%</td>
                            <td><span class="fw-bold text-success"><?= $pRecord['skor_pilar_5'] ?></span>%</td>
                            <td>
                                <span class="fw-extrabold text-success fs-6"><?= $pRecord['nilai_akhir_total'] ?></span>
                            </td>
                            <td>
                        ...
                        <?php else: ?>
                            <td colspan="7" class="text-muted text-center"><small><em>Belum dinilai</em></small></td>
                            <td>
                                <?php if (in_array($g['role'] ?? '', ['admin', 'admin_tu']) || ($g['posisi'] ?? '') === 'Admin TU'): ?>
                                    <span class="badge bg-secondary-subtle text-secondary border">-</span>
                        ...
                        <td class="text-center">
                            <div class="btn-group btn-group-sm">
                                <?php if ($g['user_id'] == session()->get('user_id')): ?>
                                    <button class="btn btn-secondary text-muted" disabled title="Anda tidak dapat menilai diri sendiri">
